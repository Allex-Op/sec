...
public class ByzantineAtomicRegisterService {
    ...
    public static synchronized void writeToRegisters(ReportDTO report, String userIdSender) throws ApplicationException {
        ...
        if(acklist.size() > (ClientApplication.numberOfServers + ByzantineConfigurations.MAX_BYZANTINE_FAULTS) / 2) {
            System.out.println("[Client " + ClientApplication.userId + "] Write Byzantine Atomic register obtained minimum quorum to execute the write request!");
    ...
    public static synchronized ReportDTO readFromRegisters(RequestLocationDTO req) throws ApplicationException {
                ...
                for (int i = 1; i <= ByzantineConfigurations.NUMBER_OF_SERVERS; i++) {
                    ...
                    CompletableFuture.runAsync(() -> {
                        try {
                            ...
                            System.out.println("[Client id:" + ClientApplication.userId+"] READ Byzantine Atomic register - Sent READCOMPLETE message to serverId: " + serverId);
                        } catch (Exception e) {
                            System.out.println(e.getMessage());
                        }
                    });
        ...
        // A byzantine quorum minimum wasn't met.
        throw new ApplicationException("Client " + ClientApplication.userId + " wasn't able to obtain at least (N+f)/2 responses for the READ atomic operation, some servers may be in an inconsistent status.");
    }
    ...
    private static SecureDTO sendMessageToServer(SecureDTO message, String url) throws UnreachableClientException {
        try {
            ...
            throw new UnreachableClientException("[Client " + ClientApplication.userId + "] Byzantine Atomic register - Wasn't able to contact server.");
        }
    }
}
